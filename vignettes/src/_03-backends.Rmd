...
* creation (matrix fill_diag example)
* skeleton objects (vectors, svd)
* printing
...
Note two other differences from the previous objects. First, there is no `mpivec()` object. There's usually no point in distributing a vector. Consider that if you have a 100,000x100,000 numeric matrix (of doubles), then that matrix would take up ~75 GiB of memory. Whereas a 100,000 length vector would use less than 1 MiB.

blocking factors

The other difference is in how we print a non-distributed object. We only want to print it on one rank, and typically rank 0 is chosen.
